...
dat <- read.csv("champi.csv", header=TRUE, sep=" ")
dat <- as.matrix(dat)

vec <- dat[10,-1]
dat <- dat[-10,-1]

# 2. Donnez une expression en R pour obtenir le vecteur des classes présentes dans *dat*.
#    Nommez ce vecteur *classes* et stockez dans une variable *colClasse* le numéro de la colonne associée à la classe dans *dat*. 
...
prVec <- function(vec, classe,data){
    return (prod(sapply(1:length(vec), function(i) prAttVal(i,vec[i],classe,data))))
}
...
names(sort(sapply(classes, prVec, vec=vec, data=dat),decreasing=TRUE))[1]
...
p <- sapply(classes, prVec, vec=vec, data=dat)
n <- p[p == max(p)]

length(n)

if(length(n)>1){
    res = names(which.max(table(dat[,colClasse]) [names(n)]))
}else res = names(n)

# 10. Écrire une fonction *classerBayes(vecteur,data)* retournant la classe attribuée au vecteur *vec*
# à partir des observations décrites dans *data* dont la dernière colonne contient les étiquettes de classes.

classerBayes <- function(vecteur,data){
    classes <- names(table(data[, ncol(data)]))
    p <- sapply(classes, prVec, vec=vecteur, data=data)
    n <- p[p==max(p)]

    if(length(n)>1){
        res = names(which.max(tables(dat[,colClasse]) [names(n)]))
    } else res = names(n)
