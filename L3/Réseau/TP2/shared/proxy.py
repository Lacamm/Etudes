...
async def pipe(reader, writer):
    while not reader.at_eof():
        data = await reader.readline(1024)
        writer.write(data)
        print(data.decode())
...
async def handle_request(reader, writer, rhost, rport):
    ...
    await asyncio.gather(pipe(reader,writer), pipe(rreader,rwriter))


async def proxy(port, rhost, rport):
...
if __name__ == '__main__':
    ...
    asyncio.run(proxy(port, rhost, rport))
