...
async def lancement_mode2(writer,reader,name,addr) : # cette fonction va verifier si le nombre de coups est reglementaire et gerer le cas d'une partie MODE: 2
    ...
    await envoi(writer,reader,"202: Table crée, en attente d'un joueur",addr)
...
async def check_name_table(writer,reader,addr) : #cette fonction verifie si le nom de la table donne par l'utilisateur est valide
    data = " "  # Verification pour savoir le nom de la table
    valide = False
    while data.isspace() or not valide: # isspace() retourne True si le str est vide ou s'il n'est compose que d'espaces, false sinon
        rcv = await recv(reader,addr)
        data = rcv[7:]
        if not data.isspace() : 
            if len(table_with_name) != 0 : 
                for user_name, nb_coups in table_with_name.items() : 
                    if user_name[2] == data : 
                        await envoi(writer,reader,"400: Choissisez un autre nom",addr)
                    else : 
                        valide = True
            else : 
                valide = True
